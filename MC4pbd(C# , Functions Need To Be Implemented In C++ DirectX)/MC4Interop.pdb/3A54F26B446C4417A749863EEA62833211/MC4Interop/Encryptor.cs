...
namespace MC4Interop
{
	internal class Encryptor
	{
		...
		{
			byte[] numArray = new byte[25];
			byte[] numArray1 = new byte[] { 0, 219, 4, 31, 217, 229, 52, 220, 41, 13, 224, 32, 221, 20, 5, 11, 248, 223, 251, 247, 5, 250, 252, 0, 0 };
			numArray[0] = 118;
			for (int i = 1; i < 24; i++)
			{
				numArray[i] = (byte)(numArray[i - 1] + numArray1[i]);
		...
		public static string PadString(string input)
		{
			...
			if (length != 0)
			{
				str = str.PadRight(input.Length + 16 - length, ' ');
			}
			return str;
		}

		public static string PadStringPHP(string input)
		{
			...
			input.PadRight(length + input.Length, ' ');
			return input;
		}
